...
export async function generate(brief: ContentBrief, opts: GenOpts = {}): Promise<GenResult> {
  ...
  if (key) {
    try {
      ...
      return normalizeResult(raw as any, brief, opts.grounding ? 'Gemini 3.7 Flash + Search' : 'Gemini 3.7 Flash');
    } catch (e: any) {
      return { pages: [], provider: 'Gemini 3.7 Flash', warnings: [e?.message ?? 'Generation failed.'] };
    }
  }
