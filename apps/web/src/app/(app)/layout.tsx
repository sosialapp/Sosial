  ...
  return (
    <ThemeScope className="app-shell min-h-screen bg-bone text-ink">
      <div className="flex min-h-screen">
        {/* Desktop rail: Postiz-style grouped sidebar. Mobile uses the dock. */}
        <Sidebar workspaceName={ctx.workspace.name} email={ctx.user.email ?? ''} />
        <div className="flex min-w-0 flex-1 flex-col">
          {/* Mobile top bar only — the sidebar carries desktop chrome. */}
          <header className="sticky top-0 z-30 flex items-center gap-2.5 border-b border-line bg-card px-4 py-3 lg:hidden">
            ...
            <Link
              href="/channels"
              className="hidden items-center gap-1.5 rounded-[10px] border border-line bg-paper px-3 py-1.5 text-xs font-bold text-soft transition hover:bg-bone sm:flex"
            >
          ...
          <main className="mx-auto min-w-0 w-full max-w-7xl flex-1 pb-32 lg:pb-10">{children}</main>
        </div>
      </div>
