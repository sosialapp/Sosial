...
function Chrome({ c, k, children, fit, maxH, watermark: wmProp }: { c: ChromeCtx; k: number; children: React.ReactNode; fit?: boolean; maxH?: number; watermark?: boolean }) {
  ...
  if (style === 'mastodon') {
    return (
      <div style={shell(10)}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 7 * k, padding: `${10 * k}px ${12 * k}px 0` }}>
          <AvatarMark page={page} size={24} k={k} />
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 * k }}>
              <span style={{ ...ff(c.font, true), fontSize: 9.5 * k, color: c.ink }}>{c.name} </span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 3 * k }}>
              <span style={{ ...ff(c.font), fontSize: 7.5 * k, color: c.gray }}>{c.firstHandle} · 2h</span>
              <ChromeIcon name="globe" size={8 * k} color={c.gray} />
            </div>
          </div>
          <ChromeIcon name="dots-v" size={12 * k} color={c.gray} />
        </div>
        <div style={bodyStyle}>{children}</div>
        <div style={{ display: 'flex', alignItems: 'center', padding: `0 ${14 * k}px ${10 * k}px`, gap: 4 * k }}>
          {[
            { icon: 'undo', count: '12', color: c.gray },
            { icon: 'repeat', count: '48', color: '#6364FF' },
            { icon: 'star', count: '312', color: '#CA8F04' },
            { icon: 'send', count: '', color: c.gray },
          ].map((a, i) => (
            <span key={i} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 3 * k }}>
              <ChromeIcon name={a.icon} size={11 * k} color={a.color} />
      ...
      <div style={shell(8)}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 7 * k, padding: `${10 * k}px ${12 * k}px 0` }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ ...ff(c.font), fontSize: 7.5 * k, color: c.gray, margin: 0 }}>{c.firstHandle}</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 3 * k }}>
              <span style={{ ...ff(c.font), fontSize: 7.5 * k, color: c.gray }}>2h ·</span>
              <ChromeIcon name="globe" size={8 * k} color={c.gray} />
            </div>
          </div>
          <ChromeIcon name="dots" size={12 * k} color={c.gray} />
        ...
        <div style={bodyStyle}>{children}</div>
        <div style={{ display: 'flex', alignItems: 'center', padding: `0 ${12 * k}px ${7 * k}px` }}>
          <span style={{ ...ff(c.font), fontSize: 8.5 * k, color: c.gray, marginLeft: 5 * k }}>48</span>
          <span style={{ flex: 1 }} />
          <span style={{ ...ff(c.font), fontSize: 8.5 * k, color: c.gray }}>12 comments · 5 reposts</span>
        </div>
        <ActionRow c={c} k={k} items={[{ icon: 'like', label: 'Like' }, { icon: 'comment', label: 'Comment' }, { icon: 'repeat', label: 'Repost' }, { icon: 'send', label: 'Send' }]} />
      </div>
    );
  }
  ...
  if (style === 'snapchat') {
    return (
      <div style={shell(16)}>
        <div style={{ ...headerRow, padding: `${11 * k}px ${13 * k}px 0` }}>
          <span style={{ position: 'relative', display: 'inline-block', width: 28 * k, height: 28 * k }}>
            <span style={{ borderWidth: 1.5 * k, borderStyle: 'solid', borderColor: '#FFFC00', borderRadius: 14 * k, padding: 1.5 * k, display: 'block' }}>
              <AvatarMark page={page} size={22} k={k} />
            </span>
            <span style={{ position: 'absolute', right: 0, bottom: 0, width: 12 * k, height: 12 * k, borderRadius: 6 * k, backgroundColor: '#FFFC00', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ChromeIcon name="ghost" size={7 * k} color="#000" />
            </span>
          </span>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 * k }}>
        ...
        <div style={bodyStyle}>{children}</div>
        <div style={{ display: 'flex', alignItems: 'center', padding: `0 ${14 * k}px ${10 * k}px`, gap: 4 * k }}>
          <span style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 3 * k }}>
            <ChromeIcon name="heart-fill" size={11 * k} color="#FF3B5C" />
            <span style={{ ...ff(c.font), fontSize: 8 * k, color: c.gray }}>12.4K</span>
          </span>
          <span style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 3 * k }}>
            <ChromeIcon name="eye" size={11 * k} color={c.gray} />
            <span style={{ ...ff(c.font), fontSize: 8 * k, color: c.gray }}>48K views</span>
          </span>
          <ChromeIcon name="send" size={11 * k} color={c.gray} />
        </div>
      </div>
    );
  }
